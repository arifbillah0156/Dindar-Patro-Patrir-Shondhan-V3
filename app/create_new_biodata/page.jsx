...
export default function Home() {
  ...
  // localStorage থেকে ডেটা লোড করার ফাংশন (সার্ভার-সাইড এরর প্রতিরোধ করার জন্য আপডেটেড)
  const loadDraftFromLocalStorage = () => {
    ...
    try {
      const savedDraft = localStorage.getItem('biodataDraft');
      if (savedDraft) {
        return JSON.parse(savedDraft);
      }
      return null;
    } catch (error) {
      console.error('ড্রাফট লোড করতে সমস্যা হয়েছে:', error);
      return null;
    }
  };

  // localStorage এ ডেটা সেভ করার ফাংশন (সার্ভার-সাইড এরর প্রতিরোধ করার জন্য আপডেটেড)
  const saveDraftToLocalStorage = (data) => {
    ...
    try {
      localStorage.setItem('biodataDraft', JSON.stringify(data));
    } catch (error) {
      console.error('ড্রাফট সেভ করতে সমস্যা হয়েছে:', error);
    }
  };

  // localStorage থেকে ড্রাফট ডিলিট করার ফাংশন (সার্ভার-সাইড এরর প্রতিরোধ করার জন্য আপডেটেড)
  const clearDraftFromLocalStorage = () => {
    ...
    try {
      ...
      console.error('ড্রাফট ডিলিট করতে সমস্যা হয়েছে:', error);
    }
  };
  ...
  const [formData, setFormData] = useState(() => {
    // কম্পোনেন্ট লোড হওয়ার সময় localStorage থেকে ডেটা লোড করার চেষ্টা করা
    const savedDraft = loadDraftFromLocalStorage();
    if (savedDraft) {
      return savedDraft;
    }

    // যদি সেভড ড্রাফট না থাকে, তাহলে ডিফল্ট ডেটা রিটার্ন করা
    return {
  ...
  // ফর্ম ডেটা পরিবর্তন হলে localStorage এ সেভ করা
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ফর্ম সাবমিট হলে localStorage থেকে ড্রাফট ডিলিট করা
  ...
  return (
    <div >
      ...
      <hr />
      <p className="mt-2"></p>
      <div className="min-h-[70vh] md:min-h-[80vh] bg-blue-50 flex items-start justify-center p-4 rounded-3xl mb-[-30px]">
        <div className="bg-white shadow-xl rounded-xl w-full max-w-lg p-6 animate-fadeIn mt-8 border border-x-0 border-purple-700 border-y-[12px] mb-12">
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit}>
              ...
              <br />
              {/* সম্মতি */}
              <p className="text-red-600  px-4 py-0 text-justify">
                <input
                  type="checkbox"
                  checked={isChecked}
                  onChange={() => {
